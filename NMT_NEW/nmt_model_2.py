...
def init_config():
    ...
    parser.add_argument('--layer_size', default=2,type=int, help='size of layers')
    parser.add_argument('--drop_out', default=0.5, type=float, help='dropout rate')
    ...
    parser.add_argument('--load_model', default="my_train_model_an-cn/model_512_unreverse_unresidual_new_split_aug.iter6.bin", type=str, help='load a pre-trained model')
    parser.add_argument('--save_to', default='my_train_model_an-cn/model_512_unreverse_unresidual_new_split_aug_layer2', type=str, help='save trained model to')
    parser.add_argument('--save_model_after', default=5, type=int,
...
def dot_prod_attention(h_t, src_encoding, src_encoding_att_linear,args):
    """
    :param h_t: (2,batch_size, hidden_size)
    :param src_encoding: (batch_size, src_sent_len, hidden_size*2)
    :param src_encoding_att_linear: (batch_size, src_sent_len, hidden_size)
    """
    # att_weight(batch_size, src_sent_len,2)
    ...
    # att_view (batch_size,2,src_sent_len)
    att_view = (att_weight.size(0), args.layer_size, att_weight.size(1))
    # ctx_vec(batch_size,2,hidden_size*2)
    ctx_vec = torch.bmm(att_weight.view(*att_view), src_encoding)
    return ctx_vec
...
"""
    ...
    def forward(self,src_sents):
        ...
        #last_state [2,batch_size,hidden_size]
        enc_output = self.input_drop(enc_output)
        last_state = F.tanh(last_cell)
        return enc_output,(last_state, last_cell)
...
class Decoder(nn.Module):
    ...
    def forward(self,src_encoding, dec_init_vec, tgt_sents,decoder_embedding,att_src_linear,att_vec_linear):
        #output[seq_len,batch_size,hidden_size*2]
        #init_state,init_cell [2,batch_size,hidden_size]
        ...
        #att_tml[2,batch_size,hidden_size]
        att_tm1 = Variable(
            init_cell.data.new(
        ...
        for one_line_embedding in tgt_embedding.split(split_size = 1):
            #one_line_embedding[2,batch_size,tgt_embedding_size]
            #att_tm1.size[2,batch_size,hidden_size]
            one_line_embedding = one_line_embedding.expand(
                att_tm1.size(0),one_line_embedding.size(1),one_line_embedding.size(2))
            input_line = torch.cat(
                [one_line_embedding,att_tm1],2)

            _,(h_t, cell_t) = self.rnn(input_line, hidden)
            # h_t[2,batch_size,hidden_size]
            h_t = self.dropout(h_t)
            #context_t[batch_size,2,hidden_size*2]
            context_t = dot_prod_attention(
                h_t, src_encoding, src_encoding_att_linear,self.args)
            att_t = F.tanh(
                att_vec_linear(torch.cat([h_t, context_t.permute(1,0,2)], 2)))
            #att_t[2,batch_size,hidden_size]
            att_t = self.dropout(att_t)
            #score_t[batch_size,vocab_size]
            score_t = self.readout(att_t[-1].squeeze(0))
...
class NMT(nn.Module):
    ...
    def translate(self, src_sent,beam_size=None, to_word=True):
        ...
        init_state = dec_init_vec[0]#[2,1,hidden_size]
        init_cell = dec_init_vec[1]#[2,1,hidden_size]
        ...
        hypotheses = [[bos_id]]
        #att_tml[2,1,hidden_size]
        att_tm1 = torch.tensor(
            torch.zeros(self.args.layer_size, len(hypotheses),self.args.hidden_size)).cuda()
        ...
        while time_step < self.args.decode_max_time_step:
            ...
            y_tm1 = torch.LongTensor([hyp[-1] for hyp in hypotheses]).cuda()
            y_tm1 = y_tm1.unsqueeze(1)
            print("***")
            print("y_tml", y_tm1.size())
            print("att_tml",att_tm1.size())
            # y_tml[2,hyp_num,embedding_dim]att_tml[2,1,hidden_size]
            y_tm1_embed = self.decoder_embedding(y_tm1)
            print("y_tml_embed1", y_tm1_embed.size())
            # y_tm1_embed = y_tm1_embed.squeeze(0)
            y_tm1_embed = y_tm1_embed.expand(att_tm1.size(0),y_tm1.size(1),y_tm1_embed.size(2))
            print("y_tml_embed2",y_tm1_embed.size())
            # x[2,hyp_num,embedding_dim+hidden_size]
            x = torch.cat([y_tm1_embed, att_tm1], 2)
            # h_t[2,hyp_num, hidden_size]
            _,(h_t, cell_t) = self.decoder.rnn(x, hidden)
            h_t = self.dropout(h_t)
            #ctx_t[1,2, hidden_size*2]
            ctx_t = dot_prod_attention(
                h_t, expanded_src_encoding.permute(1, 0, 2),
                expanded_src_encoding_att_linear.permute(1, 0, 2),self.args)
            print("ctx_t:",ctx_t.size())

            att_t = F.tanh(self.att_vec_linear(torch.cat([h_t, ctx_t.permute(1,0,2)], 2)))
            # att_t[2,1,hidden_size]
            att_t = self.dropout(att_t)
            #score_t[1,vocab_size]
            score_t = self.decoder.readout(att_t[-1].squeeze(0))

            p_t = F.log_softmax(score_t.unsqueeze(0),dim=1)
            print("softmax(score)",p_t.size())
            ...
            for prev_hyp_id, word_id, new_hyp_score in zip(prev_hyp_ids.cpu().data, word_ids.cpu().data,
                ...
                if len(live_hyp_ids) == self.args.beam_size: break
                if word_id == eos_id:
            ...
            print("live_hyp_ids",live_hyp_ids.size())
            print("h_t",h_t.size())
            hidden = (h_t,cell_t)
        ...
        ranked_hypotheses = sorted(zip(completed_hypotheses, completed_hypothesis_scores), key=lambda x: x[1],
                                   reverse=True)
        ranked_hypotheses = ranked_hypotheses[:self.args.beam_size]
...
def decode_sentence(args,model,val_iter,to_words=True,verbose=True):
    ...
    for i in range(len(src_sents_idx)):
        hyps = model.translate(src_sents_idx[i],args)
    ...
    top_hypotheses = [line.split() for line in top_hypotheses]
    top_hypotheses = [" ".join(line) for line in top_hypotheses]
...
def train(args):
    ...
    train_iter, val_iter, test_iter, an, cn =  load_data(args.batch_size,reverse=args.reverse)
    ...
    print("[EN_vocab]:%d [DE_vocab]:%d" % (args.an_vocab_size, args.cn_vocab_size))
    ...
    for epoch in range(1,args.epoch+1):
        ...
        if epoch % 1 == 0:
            ...
            save_name =  args.save_to + '.epoch%d.bin' % epoch + "output.txt"
            savefiletxt(val_gener_sents,save_name)
...
def test(args):
    ...
    savefiletxt(output_sents_words,"output_new_split_aug.txt")
