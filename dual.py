...
def dual(args):
    ...
    for i in range(2):
        ...
        print(
            '  load model{:s}     from [{:s}]'.format(model_id, args.nmt[i]),
            file=sys.stderr)
        params = torch.load(
            args.nmt[i],
        ...
        print(
            '  load train_src{:s} from [{:s}]'.format(model_id, args.src[i]),
            file=sys.stderr)
        train_srcs[model_id] = read_corpus(args.src[i], source='src')

        print(
            '  load lm{:s}        from [{:s}]'.format(model_id, args.lm[i]),
            file=sys.stderr)
    ...
    for m in ['A', 'B']:
        ...
        #         optimizers[m] = torch.optim.SGD(models[m].parameters(), lr=1e-5, momentum=0.9)
        optimizers[m] = torch.optim.Adam(models[m].parameters(), lr=args.lr)
        ...
        for t in range(start, start + len(train_srcs['A'])):
            ...
            for m in ['A', 'B']:
                ...
                for ids, smid, dist in hyps:
                    if show_log:
                        print('[smid]', str(' '.join(smid)))

                    var_ids = Variable(
                        torch.LongTensor(ids[1:]), requires_grad=False)
                    NLL_losses.append(loss_nll(dist, var_ids).cpu())

                    lm_probs.append(lmB.get_prob(smid))

                    src_sent_var = to_input_variable(
                        [smid], vocabB.src, cuda=args.cuda)
                    tgt_sent_var = to_input_variable(
                        [['<s>'] + s + ['</s>']], vocabB.tgt, cuda=args.cuda)
                    src_sent_len = [len(smid)]

                    score = modelB(src_sent_var, src_sent_len,
                                   tgt_sent_var[:-1]).squeeze(1)

                    CE_losses.append(
                        loss_ce(score, tgt_sent_var[1:].view(-1)).cpu())
                ...
                r1s = Variable(
                    torch.FloatTensor(lm_probs), requires_grad=False)
                ...
                if show_log:
                    for r1, r2, rk, fw_loss, bw_loss in zip(
                            r1s.data.numpy(), r2s.data.numpy(),
                            rks.data.numpy(), fw_losses.data.numpy(),
                            bw_losses.data.numpy()):
                        print(
                            'r1={:7.4f}\t r2={:7.4f}\t rk={:7.4f}\t fw_loss={:7.4f}\t bw_loss={:7.4f}'.
                            format(r1, r2, rk, fw_loss, bw_loss))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--nmt', nargs=2, required=True, help='pre-train nmt model path')
    parser.add_argument(
        '--lm', nargs=2, required=True, help='language model path')
    parser.add_argument(
        '--dict', nargs=2, required=True, help='dictionary path')
    parser.add_argument(
        '--src', nargs=2, required=True, help='training data path')
    parser.add_argument(
        '--model', nargs=2, type=str, default=['modelA', 'modelB'])
