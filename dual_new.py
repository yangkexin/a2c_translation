...
import argparse
import random
from NMT_NEW import nmt_model
from nmt_model import *
from lm_NEW import lm_model
form lm_model import *
...
def dual(args):
    ...
    train_iter, test_iter,val_iter, an, cn = load_data(1)
    for i in range(2):
        model_id = (['A', 'B'])[i]
        ...
        encoder[model_id] = Encoder(opts[model_id])
        decoder[model_id] = Decoder(opts[model_id])
    ...
    for m in ['A', 'B']:
        ...
        models[m] = NMT(opts[m], encoder[m], decoder[m])
        ...
        for t in range(start, start + len(train_srcs['A'])):
            ...
            for m in ['A', 'B']:
                ...
                for ids, smid, dist in hyps:
                    ...
                    var_ids = torch.LongTensor(ids[1:])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nmt', nargs=2, type=str,default=["",""], help='pre-train nmt model path')
    parser.add_argument('--lm', nargs=2, type=str,default=["",""], help='language model path')
    parser.add_argument('--dict', nargs=2, type=str,default=["",""], help='dictionary path')
    parser.add_argument('--src', nargs=2, type=str,default=["",""], help='training data path')
